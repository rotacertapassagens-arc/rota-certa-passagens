...
- **rodar `tests/outbox-claim.pg-real.test.ts` contra um PostgreSQL real e descartável** (ver o
  cabeçalho do próprio arquivo para o comando exato) antes de qualquer PostgreSQL real/staging —
  este repositório nunca instalou nem configurou PostgreSQL/Docker/WSL como parte desta tarefa;
- decidir e configurar `EMAIL_MODE=resend` explicitamente no Worker (via `wrangler.jsonc`
  vars/environment de produção) quando o envio real de e-mail for desejado — o padrão atual
  (`capture`) é intencional e idêntico ao do backend Node.
