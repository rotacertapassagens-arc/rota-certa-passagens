...
async function hasAccess(auth: Auth, env: Env) {
  if (auth.roles.includes('master')) return true;
  return !!await env.DB.prepare("SELECT 1 FROM subscriptions WHERE user_id=? AND status IN ('trialing','active') AND ends_at>CURRENT_TIMESTAMP LIMIT 1").bind(auth.userId).first();
}
...
async function verifyEmail(req: Request, env: Env) {
  ...
  await env.DB.batch([
    ...
    env.DB.prepare("INSERT INTO subscriptions(id,user_id,plan_id,status,starts_at,ends_at,provider) VALUES(?,?,?,'trialing',?,?,'internal')").bind(crypto.randomUUID(),user.id,'00000000-0000-4000-8000-000000000001',now,end),
...
async function session(req: Request, env: Env) {
  ...
  const access=auth.roles.includes('master')?{status:'master',ends_at:null,unlimited:true}:await env.DB.prepare("SELECT status,ends_at FROM subscriptions WHERE user_id=? AND status IN ('trialing','active') AND ends_at>CURRENT_TIMESTAMP ORDER BY ends_at DESC LIMIT 1").bind(auth.userId).first();
  return reply({authenticated:true,user:{id:auth.userId,email:auth.email,name:auth.name,roles:auth.roles},access:access||null});
}
...
async function acceptMaster(req: Request, env: Env) {
  ...
  await env.DB.batch([env.DB.prepare("UPDATE users SET password_hash=?,status='active',email_verified_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(await passwordHash(password),row.user_id),env.DB.prepare("INSERT OR IGNORE INTO user_roles(user_id,role) VALUES(?,'master')").bind(row.user_id),env.DB.prepare('UPDATE account_tokens SET used_at=CURRENT_TIMESTAMP WHERE id=?').bind(row.id)]);await audit(env,String(row.user_id),'admin.master_invite_accepted','user',String(row.user_id));return reply({ok:true});
}
async function requireMaster(req:Request,env:Env){const a=await getAuth(req,env);return a?.roles.includes('master')?a:null;}
async function adminOverview(req:Request,env:Env){if(!(await requireMaster(req,env)))return reply({error:'forbidden'},403);const u=await env.DB.prepare("SELECT count(*) n FROM users WHERE status<>'deleted'").first<{n:number}>();const a=await env.DB.prepare("SELECT count(*) n FROM users u WHERE u.status='active' AND (EXISTS(SELECT 1 FROM user_roles r WHERE r.user_id=u.id AND r.role='master') OR EXISTS(SELECT 1 FROM subscriptions s WHERE s.user_id=u.id AND s.status IN ('trialing','active') AND s.ends_at>CURRENT_TIMESTAMP))").first<{n:number}>();const p=await env.DB.prepare("SELECT count(*) n FROM payments WHERE status IN ('pending','processing')").first<{n:number}>();const paid=await env.DB.prepare("SELECT count(*) n FROM payments WHERE status='paid'").first<{n:number}>();return reply({users:u?.n||0,active_access:a?.n||0,pending_payments:p?.n||0,paid_payments:paid?.n||0});}
...
async function plannerGet(req:Request,env:Env,url:URL){const a=await getAuth(req,env);if(!a)return reply({error:'unauthorized'},401);if(!(await hasAccess(a,env)))return reply({error:'planner_access_expired'},402);const id=url.searchParams.get('tripId');const trip=id?await env.DB.prepare('SELECT id,name,destination,starts_on,ends_on,source FROM trips WHERE id=? AND owner_user_id=?').bind(id,a.userId).first<Row>():await env.DB.prepare('SELECT id,name,destination,starts_on,ends_on,source FROM trips WHERE owner_user_id=? ORDER BY updated_at DESC LIMIT 1').bind(a.userId).first<Row>();if(!trip)return reply({error:'trip_not_found'},404);const tripId=String(trip.id);const [it,places,expenses,budget,checklist]=await Promise.all([env.DB.prepare('SELECT id,day_number day,starts_at time,title,kind,notes FROM itinerary_items WHERE owner_user_id=? AND trip_id=? ORDER BY day_number,sort_order,starts_at').bind(a.userId,tripId).all(),env.DB.prepare('SELECT id,name,category,address,notes,latitude,longitude FROM places WHERE owner_user_id=? AND trip_id=? ORDER BY created_at').bind(a.userId,tripId).all(),env.DB.prepare('SELECT id,category,description,amount_cents,currency,spent_on FROM expenses WHERE owner_user_id=? AND trip_id=? ORDER BY created_at DESC').bind(a.userId,tripId).all(),env.DB.prepare('SELECT amount_cents,currency FROM budgets WHERE owner_user_id=? AND trip_id=?').bind(a.userId,tripId).first(),env.DB.prepare('SELECT id,text,completed,sort_order FROM checklist_items WHERE owner_user_id=? AND trip_id=? ORDER BY sort_order,created_at').bind(a.userId,tripId).all()]);return reply({trip,itinerary:it.results,places:places.results,expenses:expenses.results,budget:budget||{amount_cents:0,currency:'EUR'},checklist:checklist.results.map((x:Row)=>({...x,completed:Boolean(x.completed)}))});}
async function plannerTrip(req:Request,env:Env){const a=await mutationAuth(req,env);if(!a)return reply({error:'unauthorized'},401);if(!(await hasAccess(a,env)))return reply({error:'planner_access_expired'},402);const b=await body(req);const name=text(b?.name,120,1);if(!name)return reply({error:'invalid_trip'},400);const id=crypto.randomUUID();const travelers=Number.isInteger(b?.travelers)?Number(b?.travelers):1;await env.DB.batch([env.DB.prepare('INSERT INTO trips(id,owner_user_id,name,destination,starts_on,ends_on,travelers) VALUES(?,?,?,?,?,?,?)').bind(id,a.userId,name,text(b?.destination,180),text(b?.startsOn,10),text(b?.endsOn,10),travelers),env.DB.prepare("INSERT INTO budgets(trip_id,owner_user_id,amount_cents,currency) VALUES(?,?,0,'EUR')").bind(id,a.userId)]);return reply({id},201);}
async function ownedTrip(env:Env,userId:string,tripId:string){return !!await env.DB.prepare('SELECT 1 FROM trips WHERE id=? AND owner_user_id=?').bind(tripId,userId).first();}
async function plannerItem(req:Request,env:Env,tripId:string,kind:string,itemId?:string){const a=await mutationAuth(req,env);if(!a)return reply({error:'unauthorized'},401);if(!(await hasAccess(a,env)))return reply({error:'planner_access_expired'},402);if(!(await ownedTrip(env,a.userId,tripId)))return reply({error:'trip_not_found'},404);const b=await body(req);
