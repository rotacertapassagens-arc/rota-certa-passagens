...
async function hasAccess(userId: string, env: Env) {
  return !!await env.DB.prepare("SELECT 1 FROM subscriptions WHERE user_id=? AND status IN ('trialing','active') AND ends_at>CURRENT_TIMESTAMP LIMIT 1").bind(userId).first();
}
...
async function session(req: Request, env: Env) {
  ...
  const access=await env.DB.prepare("SELECT status,ends_at FROM subscriptions WHERE user_id=? AND status IN ('trialing','active') AND ends_at>CURRENT_TIMESTAMP ORDER BY ends_at DESC LIMIT 1").bind(auth.userId).first();
...
async function adminOverview(req:Request,env:Env){if(!(await requireMaster(req,env)))return reply({error:'forbidden'},403);const u=await env.DB.prepare("SELECT count(*) n FROM users WHERE status<>'deleted'").first<{n:number}>();const a=await env.DB.prepare("SELECT count(*) n FROM subscriptions WHERE status IN ('trialing','active') AND ends_at>CURRENT_TIMESTAMP").first<{n:number}>();const p=await env.DB.prepare("SELECT count(*) n FROM payments WHERE status IN ('pending','processing')").first<{n:number}>();const paid=await env.DB.prepare("SELECT count(*) n FROM payments WHERE status='paid'").first<{n:number}>();return reply({users:u?.n||0,active_access:a?.n||0,pending_payments:p?.n||0,paid_payments:paid?.n||0});}
...
async function plannerItem(req:Request,env:Env,tripId:string,kind:string,itemId?:string){const a=await mutationAuth(req,env);if(!a)return reply({error:'unauthorized'},401);if(!(await hasAccess(a.userId,env)))return reply({error:'planner_access_expired'},402);if(!(await ownedTrip(env,a.userId,tripId)))return reply({error:'trip_not_found'},404);const b=await body(req);
